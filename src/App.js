...
import { fetchStockMetrics, fetchStockHistory, fetchRelatedNews } from './services/stockService';

ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Title, Tooltip, Legend);

function App() {
  // console.log('apikey', process.env.REACT_APP_KEY);
  // console.log(env("REACT_APP_KEY"));
  ...
  const generateStockPitch = () => {
    ...
    const growthPotential = priceGrowth > 0 ? 'positive growth potential' : 'negative growth potential';

    let risks = [];
    stockNews.forEach(news => {
      if (news.headline.includes('decline') || news.headline.includes('drop')) {
        risks.push(`Risk: Potential decline in stock price due to negative news: ${news.headline}`);
      }
    });
  ...
  1. Based on the financial data, ${selectedStock} shows a ${peRatio > 20 ? 'high' : 'moderate'} P/E ratio, indicating that it is 
  ${peRatio > 20 ? 'potentially overvalued' : 'reasonably valued'} in the current market. The earnings per share (EPS) 
  of ${eps} suggests that the company is ${eps > 3 ? 'generating strong profits' : 'facing some profitability challenges'}. 
  The dividend yield of ${dividendYield}% reflects the company’s approach to returning value to shareholders, which is 
  ${dividendYield > 1 ? 'attractive' : 'relatively low'} for income-seeking investors.

  2. Growth Potential:
  ${growthPotential ? `The stock has shown ${growthPotential}, indicating strong momentum in the market.` : 'The stock is currently experiencing mixed performance with no clear growth trend.'}
  ...
  #Conclusion:
  In conclusion, while ${selectedStock} presents a ${peRatio > 20 ? 'risky' : 'promising'} investment opportunity, it’s 
    ...
    <div className="App container mt-5">
      ...
      {selectedStock && (
        <div className="mt-4">
          ...
          {stockMetrics ? (
            <div>
              <p><strong>Market Cap:</strong> {stockMetrics.marketCap}</p>
              <p><strong>P/E Ratio:</strong> {stockMetrics.peRatio}</p>
              <p><strong>EPS:</strong> {stockMetrics.eps}</p>
              <p><strong>Dividend Yield:</strong> {stockMetrics.dividendYield}</p>
            </div>
          ) : (
            <p>Loading stock metrics...</p>
          )}

          <button className="btn btn-primary mt-4" onClick={handleGenerateGraph}>
            Generate Graph
          </button>
